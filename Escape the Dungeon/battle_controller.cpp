...
std::vector<std::string> BattleController::handlePlayerTurn()
{
	...
	while (true)
	{
		tokens = services::command_executor::getCommandLine();
		events = services::command_executor::executeCommand(tokens, this->player_, *this->enemy_);
...
bool BattleController::startBattle()
{
	...
	while (true)
	{
		services::battle_ui::showBattleMenu(this->player_, *this->enemy_);

		// Activate effects.
		events = (this->turn_ % 2 == 0 ? this->player_.processEffects() : this->enemy_->processEffects());
		services::battle_ui::printCombatEvents(events);
